...
void lcd_setPixel( char X, char Y, char State )
{
...
void lcd_draw_line(char x1, char y1, char x2, char y2)
{
	const int16_t deltaX = abs(x2 - x1);
	  ...
	  lcd_setPixel(x2, y2, 1);

	  while (x1 != x2 || y1 != y2)
	  {
		  lcd_setPixel(x1, y1, 0);
...
void lcd_draw_ractangle(char x1, char y1, char x2, char y2)
{
...
void lcd_draw_fill_ractangle(char x1, char y1, char x2, char y2 , char state)
{
...
void lcd_putchar_p(char f, char x, char y, char size)
{
	 ...
	 for(char i=1;i<6;i++)
		 {
			 ...
			 {
				 if((charater&(1<<(j-1))))
					 lcd_draw_fill_ractangle(x+(i*size), y+(j*size), size,size ,1);
				 else
					 lcd_draw_fill_ractangle(x+(i*size), y+(j*size), size, size ,0);
			 }
...
void lcd_str_s(char x, char y, char size, const char *str) //вывод строки находящийся в оперативе
{
 char k = strlen(str);
 for(int i=0;i<k;i++)
 {
	 lcd_putchar_p(str[i], x,y,size);
	 if(size<=2)
	 {
	  x+=(5*size)+1;
	 }
